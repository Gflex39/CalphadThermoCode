...
def potfinder(temper,plot):
    ...
    for phase_name in my_phases_nbre:
        result = calculate(db_nbre, ['CU', 'NI','CU%','VA%','NI%'], phase_name, P=101325, T=temper, output='GM')
        ax.scatter(result.X.sel(component='NI'), result.GM, marker='.', s=5, color=color_dict[phase_name],linewidths=0,label=phase_name)
                    
        print(phase_name)
        temp=result.GM.values
        tempx=result.X.sel(component='NI').values
