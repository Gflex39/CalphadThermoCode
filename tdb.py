...
def tdb(filename,comps,temp,surfaces,phases,catalog=(False,"Nothing")):
    found=False
    ...
    def potfinder(temper,plot):
        ...
        for phase_name in my_phases_nbre:
            ...
            try:
                ...
                # print("Just a point")
                phase_potentials[phase_name]={'alpha':None,'beta':None,'x':x,'y':y}

            
            if catalog[0]:
                if phase_name==catalog[1]:
                    background.csvform(graphcomp[1], graphcomp[0], phase_name, temper,phase_potentials[phase_name])
            if plot:
...
tdb('Cu-Ni.tdb',['Cu', 'Ni','CU%','VA%','NI%'],1800,[],['LIQUID','FCC_A1'],catalog=(True,"LIQUID"))
